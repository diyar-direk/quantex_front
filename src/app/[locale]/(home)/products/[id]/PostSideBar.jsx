  ...
  return (
    <aside className="post-s2-sidebar">
      {data?.video && (
        <section>
          <h3>
            <FontAwesomeIcon icon={faVideo} /> {t("posts.video")}
          </h3>
          <video src={imgServerSrc(data?.video)} controls />
        </section>
      )}

      {(moreResults?.totalCount > 0 || isLoading) && (
        <section>
          <h3>
            <FontAwesomeIcon icon={faNewspaper} /> {t("posts.more_results")}
          </h3>
          {isLoading && (
          ...
          {moreResults?.data?.map((e) => (
            <Link
              href={pagesActionRouts.products(e[DBkeys.id])}
